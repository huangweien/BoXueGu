...
//保存个人资料信息
    ...
    public void saveVideoPlayList(VideoBean bean, String userName){
        if (hasVideoPlay(bean.chapterId,bean.videoId,userName)){//判断如果里面有此播放记录则需删除重新存放
            boolean isDelete = delVideoPlay(bean.chapterId,bean.videoId,userName);//删除之前的播放记录
            if (!isDelete){
                //没有删除成功时，则需跳出此方法不再执行下面语句
                return;
            }
        }
        ContentValues cv = new ContentValues();
    ...
    public boolean hasVideoPlay(int chapterId, int videoId, String userName) {
        ...
        String sql = "SELECT * FROM " + SQLiteHelper.U_VIDEO_PALY_LIST
    ...
    public boolean delVideoPlay(int chapterId, int videoId, String userName) {
        ...
        int row = db.delete(SQLiteHelper.U_VIDEO_PALY_LIST,
    ...
    //获取视频记录信息
     public List<VideoBean> getVideoHistory(String userName){
         String sql = "SELECT * FROM " + SQLiteHelper.U_VIDEO_PALY_LIST + " WHERE userName=?";
         Cursor cursor = db.rawQuery(sql, new String[]{userName});
